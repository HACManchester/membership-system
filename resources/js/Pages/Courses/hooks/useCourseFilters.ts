...
export const useCourseFilters = (courses: CourseResource[]) => {
  ...
  const filterOptions = useMemo<FilterOptions>(() => {
    ...
    const formatValues = [
      ...new Set(
        courses.filter((course) => course.format?.value).map((course) => course.format.value)
      ),
    ].sort();

    const formats = formatValues.map((value) => ({
      value,
      label: courses.find((c) => c.format?.value === value)?.format.label || value,
    }));
  ...
  const counts = useMemo<FilterCounts>(
    () => ({
      ...
      completed: courses.filter((course) => isUserTrainedForCourse(course)).length,
  ...
  const filteredCourses = useMemo(() => {
    return courses.filter((course) => {
      ...
      const completionFilter = !filters.hideCompleted || !isUserTrainedForCourse(course);
      ...
      if (filters.selectedRoom !== 'all') {
        if (course.equipment.length === 0) {
          ...
          roomFilter = course.equipment.some(
            (e) => (e.room_display || 'Not assigned to a room') === filters.selectedRoom
          );
        }
      }

      const formatFilter =
        filters.selectedFormat === 'all' || course.format?.value === filters.selectedFormat;
